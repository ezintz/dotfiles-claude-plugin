...
@test "inline scripts: a log grepped with an alternation is not a script" {
  # The `)` inside the quoted regex split the segment, leaving the log file as
  # the head word of `" /tmp/…/suite.txt` — read as running the file the first
  # command had just redirected into.
  assert_pass 'bash scripts/test.sh > /tmp/wonka/suite.txt 2>&1; grep -E "^# (pass|fail)" /tmp/wonka/suite.txt'
  assert_pass 'bash scripts/test.sh > /tmp/wonka/suite.txt 2>&1; grep -cE "(ok|not ok)" /tmp/wonka/suite.txt'
}
