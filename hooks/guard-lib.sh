    ...
    *)
      ...
      case "$t" in
        ./*|../*|/*)
          ...
          GUARD_SCRIPT_PATH="$t"; GUARD_SCRIPT_BARE=1; return 0 ;;
      esac
      return 1 ;;
  esac
}
...
guard_inline_scripts() {
  ...
  while IFS= read -r seg; do
    ...
    [ -n "$path" ] || continue
    # A bare path runs only if it is executable, and "nofs" skipped that test.
    # guard_segments cuts inside quotes, so `grep -E "(a|b)" /tmp/log.txt` leaves
    # a segment headed by the log file; paired with `… > /tmp/log.txt` earlier
    # in the same command, that read as a script written and then run. A file
    # this command creates can only become executable through a chmod in it.
    if [ "$GUARD_SCRIPT_BARE" = 1 ] && [ ! -x "$path" ]; then
      case "$raw" in *chmod*"${path#./}"*) ;; *) continue ;; esac
    fi
